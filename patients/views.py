...
def admin_upload_helssa_apk(request):
    """Upload a replacement APK from the authenticated admin panel."""

    uploaded_file = request.FILES.get("apk_file")
    if not uploaded_file:
        messages.error(request, "لطفاً یک فایل APK انتخاب کنید.")
        return redirect("admin:index")

    original_name = Path(uploaded_file.name).name
    safe_filename = get_valid_filename(original_name)
    if not safe_filename or Path(safe_filename).suffix.lower() != ".apk":
    ...
    messages.success(request, f"فایل {safe_filename} با موفقیت آپلود شد.")
    return redirect("admin:index")
